...
- Records are matched by Last Name (hyphen/space tolerant, e.g. "Chan-Lam" = "Chan Lam") and Fellowship first, then Chinese Name if it uniquely identifies someone in that bucket (checked ahead of the name match, since it's a stronger signal than a bare English first name), otherwise First/Middle Name (tolerant of full-name-in-one-field and name-order differences), with Chinese Name used to break ties when more than one master record matches by name.
...
- If that still finds nothing, matching falls back further by Last Name across *all* Fellowships, since people can legitimately belong to more than one, or master may be stale. An exact Chinese Name match is authoritative here on its own too; otherwise a strict First+Middle match (no loose tolerance) is required, with candidates whose Chinese Name disagrees excluded, and remaining ties broken by Mobile Phone/Email.
...
- If nothing above resolves and merged has a Chinese Name, the Last Name requirement is dropped entirely: matching searches all of master for a Chinese Name match (exact, or one containing the other, to handle a compound married surname prepended in one file but not the other) combined with a Mobile Phone/Home Phone/Email/Other Email match. This is the only way to find someone whose Last Name itself changed (marriage/surname change) that master hasn't been updated to reflect — every other tier requires Last Name to match and so can't find them at all. This is also the only tier that accepts Home Phone (including cross-field against the other side's Mobile Number) as a signal, since Chinese Name already narrows the search to a specific individual.
- If that still doesn't resolve, one more targeted fallback runs before giving up on scoping entirely: same Fellowship + same First Name (exact, or present as a whole word within master's First Name) + at least one strong contact signal (Mobile Phone, Home Phone, or Email), regardless of Last Name. This catches a genuinely different Last Name - a romanization variant of the same surname, or a merged-file parsing error that mistook part of a compound given name for a Last Name.
- If that didn't run or still doesn't resolve, matching falls back to the least specific tier: search all of master with no Last Name, Chinese Name, or Fellowship/First Name anchor, requiring at least 2 of {Email, Mobile Phone, Home Phone} to independently agree with the same candidate (if multiple candidates still remain, an exact Email match breaks the tie, since master has real cases of unrelated people sharing the same phone number). This is what finds a record with a completely blank Last Name, or a surname change where Chinese Name also happens to be blank.
- Email matching also checks master's `Other Email` column, not just `Email`, and tolerates a truncated email (local part before `@` must match exactly, domain may be a prefix of the other's) to handle data export glitches.
...
- Records that can't be resolved to exactly one master record (new members, nickname mismatches, incomplete data, ambiguous same-name matches, etc.) are written to `notFound.xlsx` with a `Reason` column and a `Note` column. `Note` is an automatic, unscoped scan of all of master for any partial lead (Email local part, Chinese Name overlap, or a phone number matching either master phone field) - too weak to auto-match on, but flagged for manual review. This has caught both genuine near-misses (e.g. an exact Chinese Name match only failing to auto-match because it had no phone/email to corroborate with) and data entry errors in the merged file itself (an email that actually belongs to a different, unrelated person).
