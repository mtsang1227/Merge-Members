...
- If nothing above resolves and merged has a Chinese Name, the Last Name requirement is dropped entirely: matching searches all of master for a Chinese Name match (exact, or one containing the other, to handle a compound married surname prepended in one file but not the other) combined with a Mobile Phone/Email/Other Email match. This is the only way to find someone whose Last Name itself changed (marriage/surname change) that master hasn't been updated to reflect — every other tier requires Last Name to match and so can't find them at all.
