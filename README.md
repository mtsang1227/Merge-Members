...
- If that still doesn't resolve, one more targeted fallback runs before giving up on scoping entirely: same Fellowship + same First Name (exact, or present as a whole word within master's First Name) + at least one strong contact signal (Mobile Phone or Email), regardless of Last Name. This catches a genuinely different Last Name - a romanization variant of the same surname, or a merged-file parsing error that mistook part of a compound given name for a Last Name.
...
- Email matching also checks master's `Other Email` column, not just `Email`.
