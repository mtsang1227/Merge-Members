...
Changed (yesChange.xlsx): 1272 (20 via fellowship + first name + contact info (different last name), 42 via name (different fellowship), 83 via contact info (name mismatch), 74 via chinese name + contact info (different last name), 7 via contact info (2 of 3 signals, no last name/chinese name match), 30 via contact info (different fellowship, name mismatch))
Not found (notFound.xlsx): 255 (255 no master record found) - 9 have a possible lead noted for manual review
```
...
- Matching a merged record to a master record works in stages, from most to least specific. All stages except the last two are scoped to the same Fellowship (the merged file's `Fellowship` value must equal the *last word* of master's `Fellowship` string, e.g. merged `"Esther"` matches master `"Bayview Cantonese - 2023 Esther"`) and the same `Last Name` (compared with hyphens normalized to spaces, e.g. `"Chan-Lam"` matches `"Chan Lam"`, the same tolerance already applied to First/Middle names):
  ...
  4. If step 3 also finds zero candidates, search by Last Name across *all* Fellowships (people can legitimately belong to more than one, or master may simply be stale). An exact Chinese Name match is checked first and is authoritative on its own here too (same as step 1) - Last Name already matches, only Fellowship scope is dropped, so this doesn't need the name check below. Otherwise requires a **strict** First+Middle token-set match — the "plain First Name alone is enough" tolerance from step 2 is deliberately not used here, since dropping the Fellowship scope means common first names (e.g. "Michael") can otherwise collide with unrelated people of the same last name. Candidates whose Chinese Name actively disagrees are excluded; if more than one candidate still remains, narrow further by Mobile Phone/Email. (The Chinese-first check is what resolves a case like "Billy" whose Middle Name only partially matches master's "Yiu Wah Vincent Tam" under a different Fellowship - the English name text doesn't token-match, but the full Chinese Name does, exactly.)
  ...
  7. If step 6 didn't resolve to exactly one candidate, try one more targeted fallback before giving up on scoping entirely: same Fellowship + same First Name (exact, or present as a whole word within master's First Name - e.g. merged `"Athena"` matches master's `"Athena Pui Man"`) + at least one strong contact signal (Mobile Phone, Home Phone/cross-field against Mobile, or Email/Other Email), regardless of Last Name. This catches a genuinely *different* Last Name (not just a formatting variant) - e.g. a Cantonese romanization difference like merged `"Chung"` vs master's `"Shung"` for the same surname sound, or a merged-file parsing error that mistook part of a compound given name for a Last Name (`"Athena Pui Man Ho"` split into First `"Athena"`, Last `"Man"`). Fellowship + First Name matching already narrows the search substantially before the single contact signal is even checked, making this safer than the completely unscoped tier below despite needing only one signal instead of two - the same reasoning that justifies accepting Home Phone/cross-field matching here too, same as step 6 (a real match, "Lisa Leung", was only findable because merged's Home Phone equaled master's Mobile Number).
  8. If step 7 didn't run (Chinese Name blank at step 6 and First Name didn't independently resolve at step 7) or still didn't resolve to exactly one candidate, fall back to the least specific tier: search *all* of master with no Last Name, Chinese Name, or Fellowship/First Name anchor at all, requiring at least 2 of {Email, Mobile Phone, Home Phone} to independently agree with the same candidate (each compared to its own counterpart field, not cross-field). A single matching field alone isn't reliable with nothing else to scope the search by, but two independently coincidentally matching the wrong person is very unlikely. This is what resolves a merged record with a completely blank Last Name (e.g. "Louis", Fellowship-scoped tiers can't even build a bucket key for an empty Last Name) or blank Chinese Name (e.g. "Loretta Law" whose real master record is "Loretta Mok" - Mobile Phone and Home Phone both agree, but Chinese Name is blank so step 6 never got a chance to run). If this finds more than one candidate and merged has an Email, narrow further to whichever candidate's Email also matches exactly - phone numbers have turned out to be less reliable than email in this data (master has real cases of unrelated people in the same small group sharing the exact same Home/Mobile Number, likely copied across rows by mistake), so an exact email match breaks that kind of tie safely.
  - Matches are tagged `Match Method: Name (different Fellowship)` (step 4), `Contact info (different Fellowship, name mismatch)` (step 5), `Chinese Name + contact info (different Last Name)` (step 6), `Fellowship + First Name + contact info (different Last Name)` (step 7), or `Contact info (2 of 3 signals, no Last Name/Chinese Name match)` (step 8); `Fellowship` and/or `Last Name` are added to `Changed Fields` as appropriate.
...
- Email matching tolerates a truncated/malformed email: the local part (before `@`) must match exactly, but the domain may be a prefix of the other's (e.g. merged `"kylaw99@hotmail"` matches master `"kylaw99@hotmail.com"`) - handles a real data export glitch. Deliberately narrower than a whole-string prefix check, which risks coincidentally matching unrelated people with similar local parts (e.g. `"sam@x.com"` as a prefix of `"samantha@x.com"`); requiring the local part to match exactly avoids that. This tolerance is only used to decide *whether two people are the same*, never to treat two genuinely blank emails as "matching" (that would be a huge false-positive risk) - the separate "did this field change" comparison still treats blank+blank as unchanged, same as before.
...
- Once a record is matched to exactly one master record, these fields are compared to decide "changed" vs "no change": First Name (case-insensitive — flags when the match came from token-set/fallback tolerance rather than an exact name), Middle Name, Last Name (only ever differs for a step-6/7/8 match), Fellowship (only ever differs for a step-4/5/6/7/8 match), Chinese/Chinese Name, Home Phone, Mobile Phone/Mobile Number (compared as digits only, ignoring formatting like `(647) 778-3213` vs `647-778-3213`), Email (case-insensitive), SG Leader/SG Leader (Compute), and SG Name (using the same abbreviation-tolerant comparison as the matching step, so an abbreviation difference alone isn't flagged as a change).
...
- Every `notFound.xlsx` row also gets a `Note` column from `findPossibleLeads()`: an unscoped scan of *all* of master for any partial signal (Email local part, Chinese Name exact/partial overlap, or a Mobile/Home Phone match against either master phone field) - automating the manual "does this show up anywhere, even coincidentally" check that repeatedly found both real matches and data-entry errors during spot-checking. None of these are strong enough to auto-match on (an earlier tier would already have matched if they were), so they're surfaced as leads for a human to judge, not treated as matches. Has caught genuine misses missed by hand (e.g. "Senny Tse" - an exact Chinese Name match only missed manually because of a trailing space) as well as a real data entry error in the merged file itself ("Yan-Siu Cheung" - the email `harley_xie@msm.com` on her row actually belongs to a completely different, unrelated person, "Harley ZhiQing Xie"; the local part matched but the domain has a typo, `msm.com` vs `msn.com`).
