...
Changed (yesChange.xlsx): 1237 (30 via name (different fellowship), 81 via contact info (name mismatch), 71 via chinese name + contact info (different last name), 41 via contact info (different fellowship, name mismatch))
Not found (notFound.xlsx): 290 (all "no master record found" - none ambiguous)
```
...
- Once a record is matched to exactly one master record, these fields are compared to decide "changed" vs "no change": First Name (case-insensitive — flags when the match came from token-set/fallback tolerance rather than an exact name), Middle Name, Last Name (only ever differs for a step-6 match), Fellowship (only ever differs for a step-4/5/6 match), Chinese/Chinese Name, Home Phone, Mobile Phone/Mobile Number (compared as digits only, ignoring formatting like `(647) 778-3213` vs `647-778-3213`), Email (case-insensitive), SG Leader/SG Leader (Compute), and SG Name (using the same abbreviation-tolerant comparison as the matching step, so an abbreviation difference alone isn't flagged as a change).
...
- Known limitation: nickname mismatches (e.g. merged `"Tony"` vs master `"Anthony"`), incomplete data (e.g. merged has only `"Paul"` where master's combined name is `"Paul Ching Hung"`), and a changed Last Name with blank Chinese Name (nothing left to anchor a cross-Last-Name search on) aren't resolvable by string matching alone. Simplified vs. traditional Chinese character variants (e.g. `丽` vs `麗`) also aren't normalized, so a Chinese Name match can fail on an otherwise-correct pairing if the two files used different character sets for the same name.
